...
class MetricsCollector:
    ...
    def __init__(self, redis_client: redis.Redis,
                 store=None):   # SQLiteStore (optional)
...
@app.route("/api/blackboard/reports")
def list_reports():
    """列出所有黑板报告。"""
    import glob
    files = []
    for f in sorted(glob.glob(f"{BLACKBOARD_DIR}/*.md")):
        ...
        files.append({
            ...
            "url": f"/api/blackboard/reports/{name}"
        })
    return jsonify({"reports": files, "count": len(files)})


@app.route("/api/blackboard/reports/<name>")
def get_report(name: str):
    """获取指定报告内容。支持 ?format=json|markdown|html"""
    path = os.path.join(BLACKBOARD_DIR, os.path.basename(name))
...
<style>
body {{ background:#14110F; color:#D8D4CC; font-family:system-ui,sans-serif;
  max-width:900px; margin:0 auto; padding:40px 20px; line-height:1.7; }}
h1 {{ color:#D4A843; font-size:24px; border-bottom:1px solid rgba(212,168,67,0.2); padding-bottom:10px; }}
h2 {{ color:#C87D4A; font-size:18px; margin-top:30px; }}
h3 {{ color:#E8A838; font-size:15px; }}
table {{ width:100%; border-collapse:collapse; margin:12px 0; }}
th {{ background:#2C2622; color:#D4A843; padding:8px 12px; text-align:left; font-size:13px; }}
td {{ padding:8px 12px; border-bottom:1px solid rgba(216,212,204,0.08); font-size:13px; }}
hr {{ border:none; border-top:1px solid rgba(212,168,67,0.15); margin:20px 0; }}
blockquote {{ border-left:3px solid #C87D4A; padding-left:16px; color:#C8C0B8; margin:12px 0; }}
code {{ background:#2C2622; padding:2px 6px; border-radius:3px; font-size:12px; color:#E8A838; }}
a {{ color:#D4A843; }}
</style>
</head>
<body>{html}</body>
...
def blackboard_ui():
    ...
    import glob
    files_html = ""
    for f in sorted(glob.glob(f"{BLACKBOARD_DIR}/*.md"), reverse=True):
        name = os.path.basename(f)
        size = os.path.getsize(f)
        mtime = datetime.fromtimestamp(os.path.getmtime(f))
        files_html += f"""
        <tr>
          <td><a href="/api/blackboard/reports/{name}?format=html">{name}</a></td>
          <td>{size:,} bytes</td>
          <td>{mtime.strftime("%Y-%m-%d %H:%M")}</td>
          <td><a href="/api/blackboard/reports/{name}">raw</a> | <a href="/api/blackboard/reports/{name}?format=json">json</a></td>
        </tr>"""
    if not files_html:
        files_html = '<tr><td colspan="4" style="text-align:center;color:#908880">暂无报告</td></tr>'
...
<head>
<meta charset="utf-8">
<title>Blackboard · Yaxiio</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  body{background:#14110F;color:#D8D4CC;font-family:system-ui,sans-serif;padding:40px 20px}
  .container{max-width:1000px;margin:0 auto}
  h1{color:#D4A843;font-size:22px;margin-bottom:6px}
  .sub{color:#908880;font-size:12px;margin-bottom:24px}
  table{width:100%;border-collapse:collapse}
  th{background:#2C2622;color:#D4A843;padding:10px 14px;text-align:left;font-size:13px;border-bottom:1px solid rgba(212,168,67,0.15)}
  td{padding:10px 14px;border-bottom:1px solid rgba(216,212,204,0.06);font-size:13px}
  tr:hover td{background:rgba(200,125,74,0.05)}
  a{color:#D4A843;text-decoration:none}
  a:hover{color:#C87D4A}
  .badge{display:inline-block;background:#2C2622;color:#C8C0B8;padding:2px 8px;border-radius:3px;font-size:11px;margin-left:8px}
</style>
</head>
<body>
<div class="container">
<h1>Yaxiio Blackboard <span class="badge">审计报告</span></h1>
<p class="sub">/app/.pi/blackboard/reports · {{count}} 份报告</p>
<table>
<thead><tr><th>报告</th><th>大小</th><th>修改时间</th><th>格式</th></tr></thead>
<tbody>{{files|safe}}</tbody>
</table>
<p class="sub" style="margin-top:20px">格式: html 浏览器阅读 | raw 原始Markdown | json 结构化数据</p>
</div>
</body>
</html>
""", count=len(glob.glob(f"{BLACKBOARD_DIR}/*.md")), files=files_html)
