...
> 专有名词太多？这一章用人话把每段讲一遍，建立清晰的心理图像。

---

### §1.1 真实运行路径 — "到底谁在干活？"

文档画了一条启动链条：容器一启动，同时跑了两条线。

- 左边：`Guard → yaxiio.py`，管内部任务调度
- 右边：`gateway.py`，管外部网络请求

它们各有一个 Commander，通过 Redis（一块"公共黑板"）传递消息。

**设计意图 vs 实际**：设计文档里只说"Commander 是核心引擎"，没提有两个。就像一家公司设了两个 CEO——一个管内部生产，一个管对外窗口。能工作，但状态不共享。

---

### §1.2 五层 MCP 落地状态 — "五层流水线是真的吗？"

**设计意图**：任务像流水线一样经过五个独立服务站（L1 感知 → L2 规划 → L3 调度 → L4 执行 → L5 进化），每个站是独立小服务器，通过 HTTP 通信。

**实际代码**：`workflow_engine.py` 把五个步骤全写在一个大函数里。调 MCP 服务器如果没响应，就直接用硬编码逻辑替代。

**打个比方**：你设计了一家工厂有五个独立车间，但实际上你只在总控室放了五个开关按顺序按——开关上标着"去 1 号车间"，但人和机器其实都在总控室里。

---

### §1.3 模块层与主流程脱节 — "有两套代码同时存在"

`modules/layer4/auto_scorer.py` 写了一套评分逻辑（根据代码质量打分），`workflow_engine.py` 里也写了一套（调大模型打分）。两套代码都在项目里，但从来不互相调用。就像写了两个版本的 PPT，演讲时只用了一个，另一个是废稿。

---

### §2.1 yaxiio.py — Commander 主程序

**这个文件是干嘛的**：Commander 就是"总经理"。不自己干活，只做三件事——①收任务 ②过宪法审查 ③派给流水线。

**P0 问题 —— 单实例锁无续期**：Commander 启动时在 Redis 写了个锁，120 秒过期。如果 Commander 运行超过 2 分钟，锁自动消失。Guard 看到锁没了，以为 Commander 死了，就启动第二个——结果两个 Commander 同时跑。

**比喻**：你进厕所锁了门，但这个锁每 120 秒自动弹开。外面的人看到门没锁就进来了。

---

### §2.2 constitution.py — 宪法系统

**这个文件是干嘛的**：这是 Yaxiio 最独特的设计。以前写 Agent 系统，开发者心里有一条线"什么能做、什么不能做"，全凭自觉。Yaxiio 把这条线**写成代码**：所有任务先过宪法审查，四种判决——ALLOWED（放行）、DELEGATED（走流水线）、REJECTED（拒绝）、DEGRADED（降级到沙箱里跑）。

**P0 —— compliance_rate 算错了**："合规率"应该是所有守规矩的操作（白名单+流水线）除以总数。代码里分子只算了白名单，漏了流水线——那些也是合规的。

**P1 —— 外贸业务硬编码**：宪法里禁止直接执行的操作包括 `translate_mongodb`、`generate_quote`（生成报价）——都是外贸行业特有的。但文档说自己是"通用 Agent 操作系统"。就像你说"我造了一辆通用汽车"，但方向盘上刻着"仅限外贸公司使用"。

**P2 —— 遗留的过时检查**：`check_self_execution()` 列了 7 个方法名说 Commander 绝对不能调用。但我把代码从头看到尾——这些方法根本不存在。这是 v1.04 时代的残留，就像老房子墙上贴的"此处有地雷"，但地雷早拆了。

---

### §2.3 workflow_engine.py — 核心编排

**这是整个项目最重要的文件**，1087 行。任务："收到一个活 → 拆成小活 → 分给不同 Agent → 收集结果 → 打分 → 不满意就重来"。

**设计亮点——目标自检循环**：跑完一轮后 L5 打分只有 5 分，系统不会放弃，而是分析"还差什么"→ 生成第二轮子任务 → 再跑 → 再打分，最多三轮。像严谨的工匠，做完后自己检查，不满意就继续打磨。

**P0 —— 用了一个不存在的变量**：`_do_L3_L4` 里写了 `task_id + "-" + sid`，但 `sid` 在这个位置根本没定义。简单任务走这条路会直接报错崩溃。

**比喻**：你在填一张表，有一栏写"加上编号 X"，但 X 从未被赋值。表格提交时系统崩了。

---

### §2.4 neuron.py — Agent 运行时

**这个文件是干嘛的**：neuron 是每个 Agent 的大脑。启动后 → 连 Redis 等任务 → 加载技能说明书 → 收到任务喂给大模型思考 → 提取 bash 命令 → 执行 → 结果再喂大模型分析 → 发布。

**设计亮点——Template Clone（模板克隆）**：每次任务分配独立记忆空间（`agent:审计官:task-001:memory`），做完就销毁。不同任务历史不互相污染——就像每次用一张新白纸，不在旧笔记上继续写。

**P0 —— 能力卡片功能不可用**：文档大肆宣传的"能力卡片驱动"，Commander 启动 Neuron 时根本没传卡片路径。代码有加载能力，但没有入口。就像造了带 USB 口的音响，固件支持读 U 盘，但遥控器上没有"插 U 盘"按钮。

**P0 —— shell=True 安全问题**：大模型输出的命令直接扔给终端执行。如果大模型被诱导输出 `rm -rf /`，就真的执行。有 Docker 沙箱兜底炸也只是炸容器，但安全原则是"纵深防御"——不能因为外面有墙，里面就不锁门。

---

### §2.5 gateway.py — 网关

**这个文件是干嘛的**：Yaxiio 对外的"门面"。实现了**会话与连接分离**——你关了 App（连接断开），服务器上的任务还在跑（会话存活）。重新打开，离线期间积压的消息一次性推送。这就是微信的标准设计。

**P0 —— 两个 /health 路由冲突**：给 `/health` 注册了两次，第二次覆盖第一次。为了补救，偷偷加了 `/health-old`。就像定了两个闹钟都叫"起床铃"，第二个覆盖了第一个，又设了一个"起床铃-old"。

---

### §2.6 pi_guardian_v3.py — 守护者

**这个文件是干嘛的**：Guardian 每 30 秒检查 Commander 是否活着。发现问题 → 自己读日志诊断 → 针对性修复。Commander 是总经理，Guardian 是盯着总经理的监察员。

**设计亮点——双守护互保**：那 Guardian 自己挂了怎么办？两个 Guardian（Primary 和 Secondary）通过 Redis 选举 Leader。Secondary 监视 Primary 心跳，30 秒没心跳 → 接管。就像工地上两个安全员——一个值班，一个在监控室看值班的那个。

**P1 —— 启动前无脑删锁**：Guardian 启动 Commander 前先删掉旧的锁。如果 Commander 本来就在健康运行，等于把正在运行的 Commander 的锁撕了，然后起一个新的——两个 Commander 同时跑。

---

### §2.7 层模块 (modules/layer1-5)

`modules/layer1/` 到 `layer5/`，很多文件只有 5-15 行有效代码，主流程从来不调用它们。像为了"五层架构"名头而存在的占位符。

- `model_router.py`（15 行）：3 条规则，主流程在另一个地方写了自己的模型选择
- `agent_factory.py`（15 行）：只往 Redis 写一条记录，真正的 Agent 启动在另一个地方
- `dependency_analyzer.py`（5 行）：只区分串并行，实际依赖解析在主流程里内联了
...
最严重的三个：
1. "五层都是独立 MCP Server" → 实际是空壳
2. "能力卡片驱动 Agent 创建" → 功能断路
3. "四道代码审查防火墙" → 代码里根本没找到

**诚实的地方**：Yaxiio 开发者在 `DESIGN.md` 和 `research-5layers.md` 里自己列了不足，这份坦诚在开源项目中很少见。

---

### §4 安全隐患

**最紧急**：`setup.sh` 里硬编码了 DeepSeek API Key。任何拿到代码的人都能用这个 Key 花你的钱。应立刻去 DeepSeek 后台轮换。
...
1. **宪法约束**：全世界唯一在 Agent 运行时做硬约束的系统。Anthropic 的 Constitutional AI 在训练时做，Yaxiio 在 Agent 干活的瞬间拦截——可以写论文的创新。
2. **模板克隆**：每次任务给新白纸，零上下文泄露。LangChain 的做法是"记住上一轮"，久了就膨胀。
3. **双层守护**：比 K8s 自愈更智能——K8s 只知道"挂了重启"，Guardian 能读日志诊断原因再修。
4. **bash 执行工具**：极简。大模型输出命令 → 执行 → 结果回去。不写 SDK、不定义 API。
5. **会话连接分离**：企业级 IM 设计，不是 Agent 框架该考虑的但 Yaxiio 做了。

**Yaxiio 最弱的五个地方**：

1. 五层 MCP 空壳——最大的"说了没做到"
2. 零测试——改一行不知道会不会崩
3. 两套代码平行——维护噩梦
4. 没有真正的 RAG（检索增强）
5. 硬编码外贸逻辑——和"通用系统"定位矛盾
...
| 维度 | 评分 | 一句话 |
|------|:----:|--------|
| 设计想法 | ⭐⭐⭐⭐⭐ | 五层、宪法、模克隆、自进化，概念完整 |
| 创新程度 | ⭐⭐⭐⭐⭐ | 宪法约束业界独有 |
| 代码质量 | ⭐⭐⭐ | 核心能跑，但缺测试、有 Bug |
| 文档质量 | ⭐⭐⭐⭐ | 中英双语，有诚实的自我评估 |
| 实现完成度 | ⭐⭐ | 五层 MCP 空壳，模块层脱节 |

**一句话总结**：Yaxiio 是一份有远见的设计 + 一个能跑的原型。最好的代码在 `constitution.py` 和 `pi_guardian_v3.py`，最需要重写的在 `workflow_engine.py` 和 `modules/layer*`。

---

*白话解读 — 与正文 CODE_REVIEW.md 配套阅读*
