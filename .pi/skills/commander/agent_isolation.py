...
class AgentFaultIsolation:
    ...
    def check_agent_health(self, agent_id: str) -> Tuple[str, Optional[str]]:
        ...
        # 4. 检查心跳
        last_hb = self.factory.get_agent_heartbeat(agent_id)
        if last_hb is not None:
            elapsed = time.time() - last_hb
            if elapsed > self.heartbeat_timeout_s * 3:
                return ("HARD", f"心跳严重超时: {elapsed:.0f}s")
            elif elapsed > self.heartbeat_timeout_s:
                return ("SOFT", f"心跳超时: {elapsed:.0f}s")
            elif elapsed > self.heartbeat_timeout_s * 0.5:
                return ("WARN", f"心跳延迟偏高: {elapsed:.0f}s")
    ...
    def repair_agent(self, agent_id: str) -> Dict[str, Any]:
        ...
        # ── SOFT: 尝试 PM2 restart ──
        if health in ("SOFT", "WARN"):
            print(f"[AgentFaultIsolation] 🔄 SOFT 修复 ({agent_id}): {reason}")
