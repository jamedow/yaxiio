...
publish() {
  ...
  docker exec redis-centos7 redis-cli -a 'Lt@114514!' PUBLISH "lightingmetal:agent:${target}" "${payload}" 2>/dev/null > /dev/null
}
...
docker exec redis-centos7 redis-cli -a 'Lt@114514!' SUBSCRIBE "${CHANNEL}" "${CONTROL}" 2>/dev/null | while read -r line; do
  read -r type
  read -r channel
  read -r payload
