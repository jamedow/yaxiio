...
class LLMAdapter:
    ...
    用法：
        llm = LLMAdapter(api_key="sk-xxx", base_url="https://api.deepseek.com/v1", model="deepseek-chat")
        if llm.available:
    ...
    def __init__(self, api_key: str = None, base_url: str = None, model: str = "deepseek-chat"):
        self.model = model
        self._client = None
        self._raw_client = None  # 保留原始客户端引用，兼容旧代码
...
class AsyncAgentFactory:
    ...
    async def _wait_for_agent(self, role: str, timeout: int = 5) -> bool:
...
class AgentLifecycleManagerV2:
    ...
    async def _audit_log(self, event: str, data: dict):
        """写入 MongoDB 审计日志（非阻塞 fire-and-forget）。"""
        if not self.mongo:
            return
        try:
            collection = self.mongo["lightingmetal"]["agent_audit_log"]
...
class SelfEvolvingCommander:
    ...
    async def _analyze_historical_patterns(self) -> list:
        """从 MongoDB 审计日志 + Redis 指标中提取历史模式。"""
        patterns = []
