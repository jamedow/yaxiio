...
LLM_MODEL = os.environ.get("LLM_MODEL", "deepseek-chat")  # max thinking
...
class YaxiioAgent:
    ...
    def _load_role_prompt(self) -> str:
        ...
        return prompts.get(AGENT_ROLE, f"你是 {AGENT_NAME}，一个智能 Agent。根据任务生成可执行的 shell 命令。")
...
当前任务: {task[:500]}
上下文: {json.dumps(context or {}, ensure_ascii=False)[:300]}
历史经验: {self._get_memory_summary()}

请分析这个任务，输出 JSON:
{{"reasoning": "分析思路", "plan": ["步骤1", "步骤2"], "commands": ["shell命令1", "shell命令2"]}}"""

        try:
            resp = self.llm.chat.completions.create(
                model=LLM_MODEL,
                extra_body={"thinking": {"type": "enabled", "level": "max"}},  # max thinking mode
