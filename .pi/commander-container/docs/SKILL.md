---
name: commander
description: LightingMetal多Agent系统总指挥。负责创建/调度/销毁子Agent，通过Redis Pub/Sub协调并行任务，提供Dashboard可视化，自我进化。当需要多Agent协作、并行任务、批量翻译、询价流程自动化时使用此技能。
---

# Commander — 多Agent系统总指挥 v2.0
...
docker exec redis-centos7 redis-cli -a 'Lt@114514!' PUBLISH lightingmetal:agent:售前经理 \
...
docker exec redis-centos7 redis-cli -a 'Lt@114514!' PUBLISH lightingmetal:agent:翻译官 \
...
docker exec redis-centos7 redis-cli -a 'Lt@114514!' PUBLISH lightingmetal:agent:翻译官 '{"from":"commander","to":"翻译官","type":"task","taskId":"T1"}' &
docker exec redis-centos7 redis-cli -a 'Lt@114514!' PUBLISH lightingmetal:agent:商务经理 '{"from":"commander","to":"商务经理","type":"task","taskId":"T2"}' &
docker exec redis-centos7 redis-cli -a 'Lt@114514!' PUBLISH lightingmetal:agent:售前经理 '{"from":"commander","to":"售前经理","type":"task","taskId":"T3"}' &
wait
```
...
**文件**: `/app/.pi/skills/commander/commander_v2.py`
...
**与 v1 兼容**：`commander_v2.py` 的 `main()` 函数提供与 `agent-commander.py` 完全相同的 Pub/Sub 主循环，可无缝替换。
...
| `commander_v2.py` | 集成六大优化的 Commander V2 | ✅ v2.3 |
